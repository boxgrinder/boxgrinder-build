...
module BoxGrinder
  describe RPMBasedOSPlugin do
    before(:each) do
      ...
      @config.stub!(:dir).and_return(OpenCascade[:tmp => 'tmpdir', :cache => 'cachedir'])
      @config.stub!(:os).and_return(OpenCascade[:name => 'fedora', :version => '14'])
      ...
      @appliance_config.stub!(:hardware).and_return(OpenCascade[:cpus => 1, :memory => 512, :partitions => {'/' => nil, '/home' => nil}])
      @appliance_config.stub!(:path).and_return(OpenCascade[:build => 'build/path', :main => 'mainpath'])
    ...
    describe ".build_with_appliance_creator" do
      ...
      it "should execute additional steps for Fedora 15" do
        @appliance_config.stub!(:os).and_return(OpenCascade[{:name => 'fedora', :version => '15'}])
        @appliance_config.should_receive(:default_repos).and_return(true)
