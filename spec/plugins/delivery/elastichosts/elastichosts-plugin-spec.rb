...
module BoxGrinder
  describe ElasticHostsPlugin do
    ...
    def prepare(options = {})
      ...
      @appliance_config.stub!(:hardware).and_return(OpenCascade[:cpus => 1, :arch => 'x86_64', :partitions => {'/' => {'size' => 1}, '/home' => {'size' => 2}}, :memory => 512])

      options = {
        :previous_plugin => OpenCascade[:plugin_info => {:type => :os}, :deliverables => {:disk => 'a/base/image/path.raw'}],
    ...
    context "validation" do
      describe ".validate" do
        it "should fail because we try to upload a non-base appliance" do
          lambda {
            prepare(:previous_plugin => OpenCascade[:plugin_info => {:type => :platform}])
    ...
    context "after validation" do
      ...
      describe ".upload_chunks" do
        it "should upload chunks in 2 parts" do
          @plugin.instance_variable_set(:@previous_deliverables, OpenCascade[{:disk => 'a/disk'}])
      ...
      describe ".create_server" do
        ...
        it "should create the server with 512 MB of ram for instances to be uploaded to cloudsigma" do
          ...
          @appliance_config.stub!(:hardware).and_return(OpenCascade[:cpus => 1, :arch => 'x86_64', :partitions => {'/' => {'size' => 1}, '/home' => {'size' => 2}}, :memory => 256])
