  ...
  class ApplianceImage < Rake::TaskLib
    ...
    def define
      ...
      file xml_file => [ kickstart_file, "appliance:#{@config.name}:rpms", tmp_dir ] do
        ...
        command = "sudo PYTHONUNBUFFERED=1 appliance-creator -d -v -t #{tmp_dir} --cache=#{Config.get.dir_rpms_cache}/#{@config.appliance_path} --config #{kickstart_file} -o #{appliance_build_dir} --name #{@config.name} --vmem #{@config.mem_size} --vcpu #{@config.vcpu}"
