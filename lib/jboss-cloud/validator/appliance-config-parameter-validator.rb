...
module JBossCloud
  class ApplianceConfigParameterValidator
    def validate
      ...
      raise ValidationError, "'#{ENV['JBOSS_CLOUD_OS_VERSION']}' is not a valid OS version for. Please enter valid version." if !ENV['JBOSS_CLOUD_OS_VERSION'].nil? && !Config.supported_oses[os_name].include?( ENV['JBOSS_CLOUD_OS_VERSION'] )
